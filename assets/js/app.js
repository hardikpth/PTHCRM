...
function nextLeadId() { return `L-${Math.max(2050, ...DB.pipeline.leads.map(l => +String(l.id).replace(/\D/g, '') || 0)) + 1}`; }
...
function openLeadModal(id) {
  ...
  openModal(`<div class="modal-head"><div class="modal-title">${editing ? 'Edit Lead' : 'New Lead'}</div><button class="icon-btn drawer-close" onclick="closeModal()">${I.x}</button></div>
    <div class="modal-body">
      <div class="field" id="ld-cust"><label>Client <span class="req">*</span></label><input class="input" id="ldCust" value="${esc(rec.cust)}" placeholder="e.g. Adani Infra"><div class="field-err">${I.info}Client is required</div></div>
...
let followups = (() => {
  ...
  return [
    { id:'FU-0001', leadId:'L-2049', customer:'IRCON International', subject:'Quotation negotiation follow-up', due:'2026-07-28', time:'10:30', channel:'Call', assignee:'M. Iyer', priority:'high', status:'pending', notes:'Confirm revised commercial terms.' },
    { id:'FU-0002', leadId:'L-2046', quoteNumber:'PTH/QTN/2026/0088', customer:'Reliance Industries', subject:'Purchase team response', due:'2026-07-29', time:'11:00', channel:'Email', assignee:'S. Desai', priority:'high', status:'pending', notes:'Share calibration scope clarification.' },
    { id:'FU-0003', leadId:'L-2041', customer:'Adani Infra', subject:'Requirement clarification', due:'2026-07-30', time:'15:00', channel:'Meeting', assignee:'K. Patel', priority:'med', status:'pending', notes:'Discuss bore locations and expected depth.' },
    { id:'FU-0004', leadId:'L-2047', customer:'Gujarat Metro Rail', subject:'Quotation acknowledgement', due:'2026-07-27', time:'16:00', channel:'Call', assignee:'M. Iyer', priority:'high', status:'pending', notes:'Obtain acknowledgement and technical review date.' },
    { id:'FU-0005', leadId:'L-2045', customer:'NHAI PIU Surat', subject:'Site visit confirmation', due:'2026-07-26', time:'12:00', channel:'Site Visit', assignee:'K. Patel', priority:'med', status:'completed', completedAt:'2026-07-26T12:40:00', notes:'Site visit confirmed with PIU engineer.' },
  ];
})();
...
function exportClients() {
  const rows = [['Client','Industry','Contact','Phone','Email','GST','Open Leads','Pipeline Value','Quotations','Status','Last Activity']];
  shownClients().forEach(cu => { const m = clientMetrics(cu.name), st = clientStatusOf(m), la = clientLastActivity(cu.name); rows.push([cu.name, cu.cat || '', cu.contact || '', cu.phone || '', cu.email || '', cu.gst || '', m.open, m.openValue, m.quotes, st.label, la || '']); });
...
function openClientModal(name) {
  ...
  openModal(`<div class="modal-head"><div class="modal-title">${editing ? 'Edit Client' : 'Add Client'}</div><button class="icon-btn drawer-close" onclick="closeModal()">${I.x}</button></div>
    <div class="modal-body">
      <div class="field" id="cl-name"><label>Client Name <span class="req">*</span></label><input class="input" id="clName" value="${esc(rec.name)}" placeholder="e.g. Adani Infra" ${editing ? 'readonly' : ''}><div class="field-err">${I.info}Client name is required</div></div>
...
let savedQuotations = (() => {
  ...
  return [
    {number:'PTH/QTN/2026/0091',customer:'L&T Construction',date:'2026-07-27',status:'submitted',lines:6,total:48500},
    {number:'PTH/QTN/2026/0090',customer:'Adani Infra',date:'2026-07-26',status:'review',lines:4,total:32745},
    {number:'PTH/QTN/2026/0089',customer:'Gujarat Metro',date:'2026-07-25',status:'won',lines:8,total:86412},
    {number:'PTH/QTN/2026/0088',customer:'Reliance Industries',date:'2026-07-24',status:'submitted',lines:3,total:22420},
  ];
})();
function persistQuotations(){ localStorage.setItem(QUOTATION_KEY,JSON.stringify(savedQuotations)); }
function nextQuotationNumber(){ const max=Math.max(91,...savedQuotations.map(q=>+(String(q.number).match(/(\d+)$/)?.[1]||0))); return `PTH/QTN/2026/${String(max+1).padStart(4,'0')}`; }
...
function openEnquiryModal() {
  openModal(`
    ...
    <div class="modal-body">
      <div class="field" id="e-cust"><label>Client Name <span class="req">*</span></label><input class="input" id="eCust" placeholder="e.g. Adani Infra"><div class="field-err">${I.info}Please enter a customer</div></div>
