...
function statusBadge(status) {
  const map = {
    ...
    under_process: ['badge-review','Under Process'], no_answer: ['badge-neutral','No Answer'],
...
function renderOverviewBusinessIntelligence() {
  ...
  const qg=ovGroup(quotes,x=>x.status); ovDonut('ovQuotePie','ovQuoteLegend',Object.entries(qg).map(([label,a])=>({label:quotationStatusLabel(label),value:a.length})),'Quotations');
...
function crmRuleEnabled(id){return id!=='rule-quote-approval'&&crmIntel.rules.find(r=>r.id===id)?.enabled!==false;}
...
function crmEvaluateQuotationApproval(q){return null; /* Equal-access workspace: commercial approval gates are disabled. */}
function crmDecideApproval(){toast('Approvals are not required','All five staff members have equal CRM access. Historical decisions remain in the audit trail.','info');}
...
VIEWS.intelligence=function(c){const leads=DB.pipeline.leads||[],open=leads.filter(x=>!['won','lost'].includes(x.col)),forecast=crmForecast(),ranked=open.map(l=>({l,s:crmLeadScore(l),h:crmDealHealth(l)})).sort((a,b)=>b.s.score-a.s.score),atRisk=ranked.filter(x=>x.h.level==='At Risk'),dupes=crmDuplicateClients(),pending=[];c.innerHTML=`${pageHead('CRM Intelligence','Scoring, routing, cadences, deal health, forecasting and workflow automation.',`<button class="btn btn-ghost" onclick="crmRunDailyReview()">${I.check}Run Health Review</button><button class="btn btn-primary" onclick="navigate('analytics')">${I.analytics}Open Analytics</button>`)}
  <div class="stat-strip enter"><div class="stat-chip"><div class="sc-val tnum">${ranked.filter(x=>x.s.band==='Hot').length}</div><div class="sc-label">Hot opportunities</div></div><div class="stat-chip"><div class="sc-val tnum" style="color:var(--danger)">${atRisk.length}</div><div class="sc-label">At-risk deals</div></div><div class="stat-chip"><div class="sc-val tnum">Not required</div><div class="sc-label">Approvals</div></div><div class="stat-chip"><div class="sc-val tnum">${inr(forecast.weighted)}</div><div class="sc-label">Weighted forecast</div></div><div class="stat-chip"><div class="sc-val tnum">${forecast.attainment}%</div><div class="sc-label">Target attainment</div></div><div class="stat-chip"><div class="sc-val tnum">${dupes.length}</div><div class="sc-label">Duplicate groups</div></div></div>
  ...
  <div class="grid dash-grid enter"><div class="col-6"><div class="card card-pad"><div class="card-head"><h3>Equal team access</h3></div><p>No commercial approvals are required. Every staff member can save, edit and issue quotations. Earlier decisions remain in the Audit Trail.</p></div></div>
  <div class="col-6"><div class="card card-pad"><div class="card-head"><h3>Workflow Automation</h3><span class="card-sub">Rules operate on existing CRM records</span></div>${crmIntel.rules.filter(r=>r.id!=='rule-quote-approval').map(r=>`<label class="crm-rule"><input type="checkbox" ${r.enabled?'checked':''} onchange="crmToggleRule('${r.id}',this.checked)"><span><b>${esc(r.name)}</b><small>${esc(r.condition)} → ${esc(r.action)}</small></span></label>`).join('')}</div></div></div>
...
function enterprisePermission(key){const role=enterpriseRole();if(['Director','Sales Executive'].includes(role))return true;const p=enterpriseCRM.permissions[role]||{};return p[key]===true;}
...
function managementApproval(type,record,title,amount=0,reasons=[]){return null; /* Risk information remains advisory, never blocks saving. */}
function decideManagementApproval(){crmDecideApproval();}
...
function renderManagementApprovalInbox(host){host.innerHTML='<div class="card card-pad"><h3>Approvals disabled</h3><p>All five staff members have equal access. Quotations, pricing and credit-profile changes do not require approval. Previous decisions remain in the Audit Trail.</p></div>';}
...
const QUOTATION_STATUS_OPTIONS=[['under_process','Under Process'],['won','Won'],['lost','Lost'],['no_answer','No Answer']];
function quotationStatusLabel(status){return QUOTATION_STATUS_OPTIONS.find(x=>x[0]===status)?.[1]||titleCaseSafe(String(status||'').replace(/_/g,' '));}
function normalizeQuotationStatus(status){return ['won','lost','no_answer'].includes(status)?status:'under_process';}
function unrestrictedQuotation(q){const status=normalizeQuotationStatus(q.status);return status===q.status?q:{...q,previousLifecycleStatus:q.previousLifecycleStatus||q.status,status};}
let savedQuotations = (() => {
  try { const saved=JSON.parse(localStorage.getItem(QUOTATION_KEY)); if(Array.isArray(saved)) return saved.map(unrestrictedQuotation); } catch(e) {}
  return [];
})();
...
function quotationExpiryState(q){if(['won','lost','no_answer','rejected'].includes(q.status))return 'closed';const days=Math.ceil((new Date(`${quotationValidUntil(q)}T23:59:59`)-new Date())/86400000);return days<0?'expired':days<=7?'expiring':'valid';}
VIEWS.quotations = function(c){
  const total=savedQuotations.reduce((sum,q)=>sum+(q.total||0),0),processing=savedQuotations.filter(q=>q.status==='under_process').length,won=savedQuotations.filter(q=>q.status==='won').length,lost=savedQuotations.filter(q=>q.status==='lost').length,noAnswer=savedQuotations.filter(q=>q.status==='no_answer').length;
  c.innerHTML=`${pageHead('Quotations','Lifecycle, validity, follow-ups, revisions and commercial conversion tracking.',`<button class="btn btn-ghost" onclick="openDataImport('quotations')">${I.upload}Bulk Import</button><button class="btn btn-primary" onclick="startNewQuotation()">${I.plus}Create Quotation</button>`)}
    <div class="stat-strip enter"><div class="stat-chip"><div class="sc-val tnum">${savedQuotations.length}</div><div class="sc-label">Total quotations</div></div><div class="stat-chip"><div class="sc-val tnum">${processing}</div><div class="sc-label">Under Process</div></div><div class="stat-chip"><div class="sc-val tnum" style="color:var(--primary-dark)">${won}</div><div class="sc-label">Won</div></div><div class="stat-chip"><div class="sc-val tnum" style="color:var(--danger)">${lost}</div><div class="sc-label">Lost</div></div><div class="stat-chip"><div class="sc-val tnum">${noAnswer}</div><div class="sc-label">No Answer</div></div><div class="stat-chip"><div class="sc-val tnum">${inr(total)}</div><div class="sc-label">Quotation value</div></div></div>
    <div class="filter-bar enter"><div class="filter-search">${I.search}<input placeholder="Search quotation or client..." value="${esc(quotationFilter.search)}" oninput="setQuotationFilter('search',this.value)"></div><select class="fdrop" onchange="setQuotationFilter('status',this.value)"><option value="all">All statuses</option>${QUOTATION_STATUS_OPTIONS.map(s=>`<option value="${s[0]}" ${quotationFilter.status===s[0]?'selected':''}>${s[1]}</option>`).join('')}</select><select class="fdrop" onchange="setQuotationFilter('validity',this.value)"><option value="all">All validity</option><option value="valid" ${quotationFilter.validity==='valid'?'selected':''}>Valid</option><option value="expiring" ${quotationFilter.validity==='expiring'?'selected':''}>Expiring</option><option value="expired" ${quotationFilter.validity==='expired'?'selected':''}>Expired</option></select></div>
...
function renderQuotationRegister(search){ const body=document.getElementById('quotationRegister'); if(!body)return;if(search!=null)quotationFilter.search=search;const term=quotationFilter.search.toLowerCase(); const rows=savedQuotations.filter(q=>canViewCrmRecord(q)&&(quotationFilter.status==='all'||q.status===quotationFilter.status)&&(quotationFilter.validity==='all'||quotationExpiryState(q)===quotationFilter.validity)&&(!term||`${q.number} ${q.customer} ${q.status} ${q.lostReason?.reason||''}`.toLowerCase().includes(term))).sort((a,b)=>b.number.localeCompare(a.number)); body.innerHTML=rows.length?rows.map(q=>{const validity=quotationExpiryState(q);return `<tr onclick="openQuotationDrawer('${esc(q.number)}')"><td><div class="cell-strong tnum">${esc(q.number)}</div><div class="page-desc">Valid to ${formatFollowupDate(quotationValidUntil(q))}</div></td><td class="cell-dim tnum">${formatFollowupDate(q.date)}</td><td class="cell-strong">${esc(q.customer)}</td><td class="tnum">${q.lines}</td><td class="tnum cell-strong">${maskFinancial(q.total||0)}</td><td>${statusBadge(q.status)} ${q.status==='lost'&&q.lostReason?.reason?`<div class="page-desc">${esc(q.lostReason.reason)}</div>`:validity==='expired'?'<span class="badge badge-expired">Expired</span>':validity==='expiring'?'<span class="badge badge-expiring">Expiring</span>':''}</td><td onclick="event.stopPropagation()"><div class="row-actions quotation-actions"><button class="mini-act" onclick="openQuotationDrawer('${esc(q.number)}')" title="Open">${I.eye}</button><button class="mini-act" onclick="openQuotationStatusModal('${esc(q.number)}')" title="Change status">${I.check}</button><button class="mini-act" onclick="newFollowupForQuote('${esc(q.number)}')" title="Follow-up">${I.clock}</button><button class="mini-act" onclick="duplicateQuotation('${esc(q.number)}')" title="Duplicate / revise">${I.copy||I.plus}</button><button class="mini-act" onclick="modifyQuotation('${esc(q.number)}')" title="Modify">${I.edit}</button><button class="mini-act" onclick="openQuotationSend('${esc(q.number)}')" title="Print / Send">${I.export}</button><button class="mini-act danger" onclick="deleteQuotation('${esc(q.number)}')" title="Delete">${I.x}</button></div></td></tr>`}).join(''):`<tr><td colspan="7"><div class="empty"><h4>No quotations found</h4></div></td></tr>`; }
function applyQuotationStatus(q,status,reason=''){if(!q||!QUOTATION_STATUS_OPTIONS.some(x=>x[0]===status))return false;reason=String(reason||'').trim();if(status==='lost'&&!reason)return false;const from=q.status,at=new Date().toISOString();q.status=status;q.statusUpdatedAt=at;q.statusUpdatedBy=DB.user.name;if(status==='lost')q.lostReason={reason,at,by:DB.user.name};q.statusHistory=[...(q.statusHistory||[]),{from,to:status,reason:status==='lost'?reason:'',at,by:DB.user.name}];return true;}
function openQuotationStatusModal(number){const q=savedQuotations.find(x=>x.number===number);if(!q)return;closeDrawer();openModal(`<div class="modal-head"><div><div class="modal-title">Change Quotation Status</div><div class="page-desc">${esc(q.number)} · ${esc(q.customer)}</div></div><button class="icon-btn drawer-close" onclick="closeModal()">${I.x}</button></div><div class="modal-body"><div class="field"><label>Status</label><select class="select" id="quotationStatusChoice" onchange="document.getElementById('quotationLostReasonWrap').style.display=this.value==='lost'?'block':'none'">${QUOTATION_STATUS_OPTIONS.map(x=>`<option value="${x[0]}" ${q.status===x[0]?'selected':''}>${x[1]}</option>`).join('')}</select></div><div class="field" id="quotationLostReasonWrap" style="display:${q.status==='lost'?'block':'none'}"><label>Reason for Lost <span class="req">*</span></label><textarea class="input" id="quotationLostReason" placeholder="Enter why this quotation was lost">${esc(q.lostReason?.reason||'')}</textarea><div class="page-desc">A reason is mandatory when marking a quotation Lost.</div></div></div><div class="modal-foot"><button class="btn btn-ghost" onclick="closeModal()">Cancel</button><button class="btn btn-primary" onclick="saveQuotationStatus('${esc(q.number)}')">Save Status</button></div>`);}
function saveQuotationStatus(number){const q=savedQuotations.find(x=>x.number===number),status=document.getElementById('quotationStatusChoice')?.value,reason=document.getElementById('quotationLostReason')?.value||'';if(status==='lost'&&!reason.trim()){toast('Lost reason required','Enter why the quotation was lost.','err');document.getElementById('quotationLostReason')?.focus();return;}if(!applyQuotationStatus(q,status,reason))return;persistQuotations();closeModal();toast('Quotation status updated',`${number} · ${quotationStatusLabel(status)}`,'ok');logAudit('Status Change','Quotations',`${number} → ${quotationStatusLabel(status)}${status==='lost'?` · Reason: ${reason.trim()}`:''}`);VIEWS.quotations(document.getElementById('canvas'));}
...
function saveQuotation() {
  ...
  const record={ number, revisionOf:isRevision?quotationBaseNumber(sourceNumber):(previous?.revisionOf||''), revisionNo:isRevision?Number(number.match(/\/REV(\d+)$/i)?.[1]||1):(previous?.revisionNo||0), customer, kindAttention:document.getElementById('qKindAttention')?.value.trim()||'', representative, categoryHeading:quotationCategoryHeading(), date:previous?.date||localDateISO(), validUntil:previous?.validUntil||localDateISO(validDate), status:isRevision?'under_process':normalizeQuotationStatus(previous?.status), createdBy:previous?.createdBy||DB.user.name, updatedAt:new Date().toISOString(), lastEditReason:previousSnapshot&&activeQuotationEditReason?activeQuotationEditReason.label:(previous?.lastEditReason||''), lines:quoteLines.length, total:totals.total, discount:quoteDiscountPct, terms:quoteTermsText, items:quoteLines.map(item=>({...item})) };
...
function modifyQuotation(number){const q=savedQuotations.find(item=>item.number===number);if(!q)return;pendingQuotationEditNumber=number;selectedQuotationEditReason=null;openModal(`<div class="modal-head"><div><div class="modal-title">Reason for Editing Quotation</div><div class="page-desc" style="margin-top:4px">${esc(number)} · ${esc(q.customer)}</div></div><button class="icon-btn drawer-close" onclick="cancelQuotationEditReason()">${I.x}</button></div><div class="modal-body"><p class="page-desc quotation-edit-intro">Select the reason for this modification. It will be saved in the revision history and Audit Trail.</p><div class="quotation-edit-reasons">${QUOTATION_EDIT_REASONS.map((r,i)=>`<button type="button" class="quotation-edit-reason" onclick="selectQuotationEditReason('${r.code}',this)"><span>${i+1}</span><div><b>${r.label}</b><small>${r.detail}</small></div><i></i></button>`).join('')}</div><div class="settings-warning quotation-director-note"><b>Change history</b><span>Record the edit reason for the audit trail. No approval is required.</span></div></div><div class="modal-foot"><button class="btn btn-ghost" onclick="cancelQuotationEditReason()">Cancel</button><button class="btn btn-primary" id="confirmQuotationEdit" disabled onclick="confirmQuotationEditReason()">${I.edit}Continue to Edit</button></div>`);}
...
function renderSettingsView(c) {
  ...
  c.innerHTML = `${pageHead('Settings', 'Manage your organisation, users, permissions, security and application preferences.', `<button class="btn btn-ghost" onclick="exportApplicationSettings()">${I.export}Export Settings</button>`)}
    <div class="settings-summary enter"><div><span>${I.employee}</span><b>${activeUsers}</b><small>Active users</small></div><div><span>${I.shield}</span><b>Equal</b><small>Team access</small></div><div><span>${I.branch || I.customer}</span><b>${adminSettings.branches.filter(x=>x.active).length}</b><small>Active branches</small></div><div><span>${I.check}</span><b>Saved</b><small>Browser configuration</small></div></div>
...
function renderSettingsSection(id) {
  ...
  if (id === 'roles') return `${settingsSectionHead('Team access','All five staff members have the same CRM permissions.')}<div class="card card-pad settings-panel"><p>Directors and the Sales Executive can view and manage all CRM records, rates, financial information and settings. No commercial approvals are required.</p><p>Job titles remain descriptive. Individual sign-in, tenant isolation and the Audit Trail remain enabled.</p></div>`;
