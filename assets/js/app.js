...
function startNewQuotation(customer=''){ quoteLines=[]; quoteDiscountPct=0; quoteTermsKey='material'; quoteTermsText=QUOTE_TERMS.material; quoteKindAttention=''; editingQuotationNumber=''; editingQuotationCustomer=customer||''; activeQuotationEditReason=null; navigate('createquotation'); }
...
function saveQuotation() {
  ...
  const record={ number, customer, kindAttention:document.getElementById('qKindAttention')?.value.trim()||'', representative, categoryHeading:quotationCategoryHeading(), date:previous?.date||localDateISO(), validUntil:previous?.validUntil||localDateISO(validDate), status:previous?.status||'review', createdBy:previous?.createdBy||DB.user.name, updatedAt:new Date().toISOString(), lastEditReason:previousSnapshot&&activeQuotationEditReason?activeQuotationEditReason.label:(previous?.lastEditReason||''), lines:quoteLines.length, total:totals.total, discount:quoteDiscountPct, terms:quoteTermsText, items:quoteLines.map(item=>({...item})) };
  ...
  persistQuotations();crmRunAutomation('quotation_saved',savedQuotations.find(q=>q.number===number)||record); resetQuotationFilters(); toast('Quotation saved', `${number} · ₹${totals.total.toLocaleString('en-IN')} incl. GST`); logAudit(existing>=0?'Edit':'Create','Quotations',`${number} saved for ${customer}${previousSnapshot&&activeQuotationEditReason?` · Reason: ${activeQuotationEditReason.label}`:''}`); activeQuotationEditReason=null; navigate('quotations');
}
